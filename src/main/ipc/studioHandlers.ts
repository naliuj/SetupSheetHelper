...
export function registerStudioHandlers(): void {
  ...
  ipcMain.handle(IPC.studios.createCustom, (_e, name: string, folderId: number | null, hasConsole: boolean) =>
    studiosRepo.createCustomStudio(name, folderId, hasConsole)
  )
  ipcMain.handle(IPC.studios.createTemporary, () => studiosRepo.createTemporaryStudio())
  ipcMain.handle(
    IPC.studios.updateCustomDetails,
    (_e, id: number, name: string, folderId: number | null, hasConsole: boolean) =>
      studiosRepo.updateCustomStudio(id, name, folderId, hasConsole)
  )
